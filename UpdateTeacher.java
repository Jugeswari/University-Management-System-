...
public class UpdateTeacher extends JFrame implements ActionListener{
    ...
    public UpdateTeacher()
    {
     ...
     getIdValue = ViewTeacherDetails.getValue();
//     JOptionPane.showMessageDialog(null,getIdValue);          
     setVisible(true);
     setValue();
    ...
    public void actionPerformed(ActionEvent ae){
        ...
        if(ae.getSource()==b1){
            try{
                conn cc =new conn();
                String q= "update teacher set Name = '"+namet.getText()+"',Email = '"+email.getText()+"', Age = '"+aget.getText()+"', DOB = '"+dobt.getText()+"', Gender = '"+gendert.getText()+"', Phone_No = '"+phonenot.getText()+"', Education = '"+(String)c2.getSelectedItem()+"', Department = '"+(String)c1.getSelectedItem()+"' where Teacher_Id = '"+getIdValue+"'";
    ...
    public void  setValue(){
        try{
            conn con = new conn();
            String str = "select * from teacher where Teacher_Id = '"+getIdValue+"'";
            ResultSet rs = con.s.executeQuery(str);

            if(rs.next()){
                namet.setText(rs.getString(1));
                emailt.setText(rs.getString(2));
